...
def vms_only(
    ...
    user_ids: str = typer.Option(
        "1,2,3,4,5", "--user-ids", "-u", help="IDs des utilisateurs (séparés par des virgules)"
    ),
    output_file: str = typer.Option(
    ...
    try:
        ...
        typer.echo(f"🖥️ Génération de {vm_count} VMs pour les utilisateurs {user_ids_list}...")
...
def preview(
    ...
    """
    typer.echo(f"👀 Prévisualisation de {user_count} utilisateurs avec 0-{max_vms} VMs chacun...")
    
    try:
        # Générer les données pour prévisualisation
        users_data = DataGenerator.generate_users_with_vms(
            user_count=user_count,
            vm_per_user_range=(0, max_vms)
        )
        ...
        for i, user in enumerate(users_data, 1):
            ...
            if user['vms']:
                ...
                for j, vm in enumerate(user['vms'], 1):
                    ...
                    typer.echo(f"         CPU: {vm['cpu_cores']} cœurs, RAM: {vm['ram_gb']}GB, Disque: {vm['disk_gb']}GB")
