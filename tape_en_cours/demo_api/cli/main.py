...
Exemples d'utilisation:
  ...
  %(prog)s vm create --email user@example.com --cores 4 --ram 8
    ...
    vm_parser = subparsers.add_parser(
        "vm",
        help="Opérations sur les VMs"
    )
    
    vm_subparsers = vm_parser.add_subparsers(
        dest="vm_command",
        help="Actions sur les VMs",
        required=True
    )
    
    # Sous-commande 'vm create'
    create_parser = vm_subparsers.add_parser(
        "create",
        help="Créer une VM"
    )
    ...
    create_parser.add_argument(
        "--name",
        default="VM de Jean",
        help="Nom de la VM (défaut: VM de Jean)"
    )
    create_parser.add_argument(
        "--os",
    ...
    create_parser.add_argument(
        ...
        type=int,
        default=2,
        help="Nombre de cœurs CPU (défaut: 2)"
    )
    create_parser.add_argument(
        "--ram",
        type=int,
        default=4,
        help="RAM en GB (défaut: 4)"
    )
    create_parser.add_argument(
        "--disk",
        type=int,
        default=50,
        help="Disque en GB (défaut: 50)"
    )
    create_parser.add_argument(
        "--status",
        default="stopped",
        help="Statut initial de la VM (défaut: stopped)"
    )
...
def main():
    ...
    if args.command == "report":
        ...
        logger.error("Commande inconnue", command=args.command, vm_command=getattr(args, 'vm_command', None))
