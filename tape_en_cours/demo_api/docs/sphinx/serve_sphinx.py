#!/usr/bin/env python3
"""
Serveur HTTP simple pour servir la documentation Sphinx.

Usage: python serve_sphinx.py [port]
"""
...
def main():
    ...
    docs_dir = Path(__file__).parent / "build"

    if not docs_dir.exists():
        print(f"❌ Erreur: Le répertoire {docs_dir} n'existe pas.")
        print("💡 Exécutez d'abord: sphinx-build -b html source build")
        print("💡 Ou utilisez: python ../../scripts/generate_docs.py")
        sys.exit(1)
    ...
    with socketserver.TCPServer(("", port), CustomHTTPRequestHandler) as httpd:
        print(f"🚀 Serveur de documentation Sphinx démarré")
