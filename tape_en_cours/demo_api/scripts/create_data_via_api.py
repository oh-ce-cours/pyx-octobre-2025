...
def full_dataset(
    ...
    try:
        ...
        if not api_client.is_authenticated():
            console.print(
                "[bold yellow]⚠️ Aucun token disponible, tentative d'authentification automatique...[/bold yellow]"
            )
            try:
                ...
                if config.has_credentials:
                    ...
                    console.print(
                        "[bold green]✅ Authentification réussie ![/bold green]"
                    )
                else:
                    console.print(
                        "[bold red]❌ Aucun identifiant configuré pour l'authentification[/bold red]"
                    )
                    console.print(
                ...
                console.print(
                    f"[bold red]❌ Échec de l'authentification: {auth_error}[/bold red]"
                )
                raise typer.Exit(1)
        ...
        if created_users is None:
            ...
            console.print(
                Panel.fit(
                    "[bold red]❌ Erreur: created_users est None[/bold red]\n"
                    "[dim]💡 Problème dans la création des utilisateurs[/dim]",
                    border_style="red",
                )
            )
            raise typer.Exit(1)
