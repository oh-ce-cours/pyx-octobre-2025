...
class DataCleanup:
    ...
    def get_all_users(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                "Erreur lors de la récupération des utilisateurs", error=str(e)
            )
            raise
    ...
    def delete_vm(self, vm_id: int, vm_name: str = None) -> bool:
        ...
        try:
            ...
            logger.info(
                "VM supprimée avec succès", vm_id=vm_id, vm_name=vm_name, result=result
            )
            return True
    ...
    def delete_user(
        self, user_id: int, user_name: str = None, user_email: str = None
    ) -> bool:
        """
    ...
    def cleanup_everything(self, confirm: bool = False) -> Dict[str, Any]:
        ...
        total_stats = {
            ...
            "summary": {
                ...
                "deleted_items": vms_stats["deleted_vms"]
                + users_stats["deleted_users"],
                "failed_items": vms_stats["failed_deletions"]
                + users_stats["failed_deletions"],
                "success_rate": 0,
            },
        }
        ...
        if total_stats["summary"]["total_items"] > 0:
            total_stats["summary"]["success_rate"] = (
                ...
                / total_stats["summary"]["total_items"]
                * 100
            )

        logger.info("✅ Nettoyage terminé", **total_stats["summary"])
...
def print_stats(stats: Dict[str, Any]):
    ...
    if vms_stats.get("vm_details"):
        ...
        for vm in vms_stats["vm_details"]:
            print(
                f"     • ID {vm.get('id')}: {vm.get('name')} (User: {vm.get('user_id')})"
            )
    ...
    if users_stats.get("user_details"):
        ...
        for user in users_stats["user_details"]:
            print(
                f"     • ID {user.get('id')}: {user.get('name')} ({user.get('email')})"
            )
...
def main():
    ...
    parser.add_argument("--base-url", help="URL de base de l'API", default=None)
    parser.add_argument("--email", help="Email pour l'authentification")
    ...
    parser.add_argument(
        "--vms-only", action="store_true", help="Supprime seulement les VMs"
    )
    parser.add_argument(
        "--users-only", action="store_true", help="Supprime seulement les utilisateurs"
    )
    parser.add_argument(
        "--wait",
        type=int,
